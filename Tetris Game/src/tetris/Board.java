...
import tetris.TetrisGame.TetrisGaming;
...
public class Board extends JPanel implements ActionListener {
    ...
    // current
    private TetrisGame curBlock;

    // logical game block
    private TetrisGaming[] gameBoard;
    ...
    private Window tetrisFrameD;


    public Board(Window tetrisFrame, int timerResolution) {

        setFocusable(true);
        setBackground(new Color(0, 30, 30));
        curBlock = new TetrisGame();
        timer = new Timer(timerResolution, this);
        timer.start();    // start timer
        currentTimerResolution = timerResolution;

        gameBoard = new TetrisGaming[BoardWidth * BoardHeight];
        ...
        addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                if (!isStarted || curBlock.getShape() == TetrisGaming.NO_BLOCK) {
                    return;
                }
    ...
    private void initBoard() {
        for (int i = 0; i < BoardWidth * BoardHeight; i++) {
            gameBoard[i] = TetrisGaming.NO_BLOCK;
        }
    }
    ...
    */
    ...
    // get board size Height and Width
    ...
    TetrisGaming curTetrominoPos(int x, int y) {
        return gameBoard[(y * BoardWidth) + x];
    }


    /*
    Game level and Score presentation
     */
    @Override
    public void paint(Graphics g) {
        ...
        g.setFont(new Font("Consola", Font.PLAIN, 28));
        ...
        for (int i = 0; i < 4; i++) {
            ...
            drawTetrisGame(g, 0 + x * blockWidth(), boardTop + (BoardHeight - y - 1) * blockHeight(), curBlock.getShape(),
                    true);
        }
        ...
        for (int i = 0; i < BoardHeight; i++) {
            for (int j = 0; j < BoardWidth; j++) {
                TetrisGaming shape = curTetrominoPos(j, BoardHeight - i - 1);
                if (shape != TetrisGaming.NO_BLOCK)
                    drawTetrisGame(g, 0 + j * blockWidth(), boardTop + i * blockHeight(), shape, false);
            }
        }


        // rendering - current game
        if (curBlock.getShape() != TetrisGaming.NO_BLOCK) {
            for (int i = 0; i < 4; i++) {
                int x = curX + curBlock.getX(i);
                int y = curY - curBlock.getY(i);
                drawTetrisGame(g, 0 + x * blockWidth(), boardTop + (BoardHeight - y - 1) * blockHeight(),
    ...
    private void drawTetrisGame(Graphics g, int x, int y, TetrisGaming bs, boolean isShadow) {
    ...
    private void removeFullLines() {
        ...
        for (int i = BoardHeight - 1; i >= 0; i--) {
            ...
            for (int j = 0; j < BoardWidth; j++) {
                if (curTetrominoPos(j, i) == TetrisGaming.NO_BLOCK) {
                    isFull = false;
                    break;
                }
            }
        ...
        if (fullLines > 0) {
            ...
            isFallingDone = true;
            curBlock.setShape(TetrisGaming.NO_BLOCK);
            setResolution();
            repaint();
        }
    ...
    //current position set true and shadow position set false
    private boolean atomIsMovable(TetrisGame chkBlock, int chkX, int chkY, boolean flag) {
        for (int i = 0; i < 4; i++) {
            int x = chkX + chkBlock.getX(i);
            int y = chkY - chkBlock.getY(i);
            if (x < 0 || x >= BoardWidth || y < 0 || y >= BoardHeight)
                return false;
            if (curTetrominoPos(x, y) != TetrisGaming.NO_BLOCK) {
                return false;
            }
        }
    ...
    private boolean isMovable(TetrisGame chkBlock, int chkX, int chkY) {
    ...
    private void writeDB(int dbScore) {
        try {
            ...
            outputStream.write("database game ");
            outputStream.close();
        } catch (IOException e) {
